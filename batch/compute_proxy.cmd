...
rem set europe_list=ALB ANR AUT BLR BEL BIH BGR HRV CZE DNK EST FRO FIN FRA DEU GRC GGY HUN ISL IRL IMN ITA JEY KOS LVA LIE LTU LUX MLT MDA MCO MNE NLD MKD NOR POL PRT ROU ERUS SMR SRB SVK SVN ESP_EUR SJM SWE CHE UKR GBR VAT ALA TUR VCS CYP
...
set europe_list=CYP ESP_EUR
set africa_list=ESP_AFR
set north_america_list=TTO
...
for %%c in (%europe_list%) do (
	...
	C:\%pf%\ObjectVision\%geodmsversion%\GeoDmsRun.exe %config%\main.dms /Analyses/ComputeProxy/Per_Continent/Europe/country_proxy/%%c/Results_per_distdecay/Geurs_VanEck_2003_general/store1
	C:\%pf%\ObjectVision\%geodmsversion%\GeoDmsRun.exe %config%\main.dms /Analyses/ComputeProxy/Per_Continent/Europe/country_proxy/%%c/Results_per_distdecay/Geurs_VanEck_2003_general/store2
)
